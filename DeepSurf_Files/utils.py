...
def mol2_reader(mol_file):  # does not handle H2
    # Reads the content of a .mol2 file and extracts the atom information
    if mol_file[-4:] != 'mol2':
    ...
    # Find the indices for the atom section
    ...
    # Return the lines containing atom information
...
def readSurfPoints(surf_file):
    ...
    # Filter lines to only include those with sufficient data
    ...
    # Initialize arrays for coordinates and normals
    coords = np.zeros((len(lines), 3))
    normals = np.zeros((len(lines), 3))
    for i, l in enumerate(lines):
        parts = l.split()
        try:
            # Try to extract coordinates and normals
...
#Function used in Protein.py
def simplify_dms(init_surf_file, factor, seed=None):
    # Simplifies the surface points by reducing the number of points using clustering
    ...
    # Use KMeans clustering to reduce the number of points
...
def rotation(n):
    # Computes the rotation matrix to align a normal vector with the z-axis
